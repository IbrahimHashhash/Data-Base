...
// Handle exceptions accordingly
    ...
    public List<Customer> getCustomersWithDownPayment() {
        List<Customer> customersWithDownPayment = new ArrayList<>();
        String sql = "SELECT DISTINCT c.cus_Id, c.first_name, c.last_name, c.birth_date, c.driving_license, c.age, c.phone, c.address, c.gender " +
                "FROM Payment p " +
                "JOIN Customer c ON p.cus_Id = c.cus_Id " +
                "WHERE p.down_payment <> 0 AND p.down_payment != p.total_cost";
        try (PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
                int customerId = resultSet.getInt("cus_Id");
                ...
                customersWithDownPayment.add(customer);
            }
        } catch (SQLException e) {
            e.printStackTrace();
            // Handle exceptions accordingly
        }
        return customersWithDownPayment;
    }
